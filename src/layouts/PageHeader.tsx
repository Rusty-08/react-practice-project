  ...
  return (
    ...
    <>
      ...
      >
        <div
          className={`${
            viewFullSearch ? "hidden" : "flex"
          } md:gap-3 gap-2 h-full items-center flex-shrink-0`}
        >
          <Button onClick={showSidebar} variant="ghost" size="icon">
            <Menu strokeWidth={1} />
          </Button>
          <a title="TubeTube Home" href="/" className="flex items-center gap-2">
        ...
        <div
          className={`${
            viewFullSearch ? "flex" : "md:flex hidden"
          } flex-grow relative items-center justify-end`}
        >
          <Button
          ...
          <form action="" className="flex flex-grow items-center">
            <div className="relative flex flex-grow items-center">
              ...
              {searchContent !== "" && (
                <Button
                  type="button"
                  onClick={(e) => eraseInputValue(e)}
                  className="absolute w-9 h-9 right-[2px]"
                  variant="ghost"
                  size="icon"
                >
                  <X strokeWidth={1} />
              ...
              <Search
                strokeWidth={1}
                className="invisible peer-focus:lg:visible absolute z-10 left-5 w-5"
              />
            </div>
            <Button
              ...
              variant="ghost"
              size="icon"
              className="flex-shrink-0 rounded-none py-1.5 bg-neutral-100 border border-secondary-border border-s-0 rounded-e-full w-16"
            >
        ...
        <div
          className={`${
            viewFullSearch ? "hidden" : "flex"
          } items-center gap-1 md:gap-2 md:me-4 flex-shrink-0`}
        >
          <Button
            onClick={() => setViewFullSearch(true)}
            variant="ghost"
            size="icon"
